...
(function() {
  ...
  function createElement(tag, className, content, attrs) {
    ...
    if (className) {
      if (Array.isArray(className)) {
        element.classList.add.apply(element.classList, className.filter(Boolean));
      } else {
  ...
  function createSVGText(text, x, y, attrs) {
    var textElement = createSVGElement('text', Object.assign({ x: x, y: y }, attrs || {}));
    textElement.textContent = text;
    return textElement;
  }

  function createSVGRect(x, y, width, height, attrs) {
    return createSVGElement('rect', Object.assign({ x: x, y: y, width: width, height: height }, attrs || {}));
  }

  function createSVGLine(x1, y1, x2, y2, attrs) {
    return createSVGElement('line', Object.assign({ x1: x1, y1: y1, x2: x2, y2: y2 }, attrs || {}));
  }
  ...
  function renderTableSection(section, periodData, viewMode) {
    var sectionEl = createElement('section', 'table-section ' + section.cssClass);
  ...
  function renderBarChart(config, data) {
    ...
    var gridLines = generateGridLines(scale, chartTop, chartBottom, chartLeft, chartRight);
    ...
    data.forEach(function(item, index) {
      var value = customValues && customValues[index] !== undefined
        ? customValues[index]
        : item.value;
  ...
  var FUND_ICONS = {
    default: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 3L4 9v12h16V9l-8-6zm6 16H6v-9l6-4.5 6 4.5v9z"/><rect x="8" y="13" width="3" height="3"/><rect x="13" y="13" width="3" height="3"/><rect x="8" y="17" width="3" height="2"/><rect x="13" y="17" width="3" height="2"/></svg>',
    warehouse: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.66 1.34 3 3 3s3-1.34 3-3h6c0 1.66 1.34 3 3 3s3-1.34 3-3h2v-5l-3-4zM6 18.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm13.5-9l1.96 2.5H17V9.5h2.5zm-1.5 9c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg>',
    office: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 7V3H2v18h20V7H12zM6 19H4v-2h2v2zm0-4H4v-2h2v2zm0-4H4V9h2v2zm0-4H4V5h2v2zm4 12H8v-2h2v2zm0-4H8v-2h2v2zm0-4H8V9h2v2zm0-4H8V5h2v2zm10 12h-8v-2h2v-2h-2v-2h2v-2h-2V9h8v10zm-2-8h-2v2h2v-2zm0 4h-2v2h2v-2z"/></svg>'
  };

  function getFundIcon(fundName) {
    var iconMap = {
      'ДВН': 'default', 'ЗОЛЯ': 'office', 'КРАС': 'warehouse',
      'ЛОГ': 'warehouse', 'НОР': 'default', 'ОЗН': 'office',
      'ТРМ': 'warehouse', 'СБЛ': 'office'
    };
  ...
  var FundSelector = {
    ...
    elements: { container: null, trigger: null, value: null, dropdown: null, grid: null },

    init: function(options) {
      options = options || {};
      this.state.selected = options.selected || (window.DATA && window.DATA.currentFund) || 'ДВН';
      this.state.funds = options.funds || (window.DATA && window.DATA.funds) || ['ДВН', 'ЗОЛЯ', 'КРАС', 'ЛОГ', 'НОР', 'ОЗН', 'ТРМ', 'СБЛ'];
    ...
    createFundItem: function(fund) {
      ...
      item.addEventListener('click', function() { self.select(fund); });
  ...
  var ViewToggle = {
    ...
    init: function(options) {
      options = options || {};
      this.state.mode = options.mode || (window.DATA && window.DATA.viewMode) || 'details';
      this.state.onChange = options.onChange || function() {};
    ...
    attachEvents: function() {
      var self = this;
      if (this.elements.button) {
        this.elements.button.addEventListener('click', function() { self.toggle(); });
      }
    },
  ...
  var Dashboard = {
    ...
    initComponents: function() {
      ...
      FundSelector.init({
        ...
        funds: (window.DATA && window.DATA.funds) || ['ДВН', 'ЗОЛЯ', 'КРАС', 'ЛОГ', 'НОР', 'ОЗН', 'ТРМ', 'СБЛ'],
        onChange: function(fund) { self.handleFundChange(fund); }
      });

      ViewToggle.init({
        mode: this.state.viewMode,
        onChange: function(mode) { self.handleViewModeChange(mode); }
      });
    },
    ...
    renderChart: function() {
      ...
      var chartData = this.state.periods.map(function(period, index) {
        return {
          label: period.label.indexOf('\n') !== -1 ? "План '" + (period.shortLabel || '25') : period.label,
          value: (this.state.chartConfig.customValues && this.state.chartConfig.customValues[index]) || 0,
          type: period.type
        };
      }, this);
    ...
    updateData: function(newData) {
      ...
      if (newData.chartConfig) {
        this.state.chartConfig = Object.assign({}, this.state.chartConfig, newData.chartConfig);
      }
