...
```javascript
// Одна команда — загрузить данные и обновить дашборд
...
**Что делает `loadData()`:**
1. Сохраняет данные в `state.rawApiData`
2. Извлекает список фондов из ключей объекта `data`
3. Устанавливает первый фонд как текущий
4. Обновляет выпадающий список фондов (FundSelector)
5. Генерирует и рендерит данные

### Тест выравнивания дельт

Для проверки выравнивания процентов в режиме "Динамика по годам" с разными дельтами (от 1% до -100%):
...
BDDS.Dashboard.loadData({
  data: {
    "Тест": {
      "Факт '23": {
        "Движение Д/С по операционной деятельности ООО": 1000000,
        "Поступления по операционной деятельности": 1000000,
        "Расходы по основной деятельности": 1000000,
        "Обеспечительные платежи": 1000000,
        "Движение Д/С по инвестиционной деятельности": 1000000,
        "Выплата дохода акционерам (пайщикам)": 1000000,
        "Расходы на капитальный ремонт": 1000000,
        "Движение Д/С по финансовой деятельности": 1000000,
        "Расчеты по кредитам": 1000000,
        "Прочие доходы и расходы по финансовой деятельности": 1000000,
        "Расходы на УК Парус": 1000000,
        "Остаток по кредиту на начало периода": 1000000,
        "Остаток по кредиту на конец периода": 1000000,
        "Движение за период по Д/С": 1000000
      },
      "План '26": {
        "Движение Д/С по операционной деятельности ООО": 1010000,
        "Поступления по операционной деятельности": 1300000,
        "Расходы по основной деятельности": 2000000,
        "Обеспечительные платежи": 990000,
        "Движение Д/С по инвестиционной деятельности": 700000,
        "Выплата дохода акционерам (пайщикам)": 500000,
        "Расходы на капитальный ремонт": 0,
        "Движение Д/С по финансовой деятельности": 1500000,
        "Расчеты по кредитам": 1050000,
        "Прочие доходы и расходы по финансовой деятельности": 1990000,
        "Расходы на УК Парус": 900000,
        "Остаток по кредиту на начало периода": 1100000,
        "Остаток по кредиту на конец периода": 600000,
        "Движение за период по Д/С": 100000
      }
    }
  }
});
...
Переключитесь на "Динамика по годам" — дельты должны быть выровнены: `(1%)`, `(30%)`, `(100%)`, `(-1%)`, `(-30%)`, `(-50%)`, `(-100%)` и т.д.
