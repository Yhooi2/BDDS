...
**Важно:** Период типа `План` обязателен для каждого фонда.
...
BDDS.Dashboard.updateData({ viewMode: "dynamics" });
```
...
BDDS.METRIC_KEYS.INVESTMENT_ADDITIONAL_SHARES // "Доходы от доп.выпуска паев"
  ...
  funds: {
    "Тест1": {
      ...
      periods: {
        ...
        "План '25": { "Поступления по операционной деятельности": 6000 }
      }
    }
  }
});
...
// Визуально: в таблице должно появиться "5 678" и "6 000"
```
  ...
  funds: {
    "Фонд1": {
      ...
      periods: {
        ...
        "План '25": { "Поступления по операционной деятельности": 60 }
      }
    },
    "Фонд2": {
      type: "warehouse",
      periods: {
        ...
        "План '25": { "Поступления по операционной деятельности": -700 }
      }
    }
  }
});
...
// Визуально: в таблице должно быть "-789" и "-700"
```
...
График автоматически строится на основе данных метрики "Выплата дохода акционерам (пайщикам)" из текущего фонда.

```javascript
// Проверить данные графика для текущего фонда
var state = BDDS.Dashboard.getState();
var metric = state.chartConfig.metric;
state.periodData.forEach(function(p, i) {
  console.log(p.title + ": " + Math.abs(p.metrics[metric] || 0));
});

// Ожидаемый вывод для ДВН:
// Факт '23: 150
// Факт '24: 480
// Факт (I-III кв. '25)...: 920
// План '26: 1350

// Переключить на другой фонд и проверить
BDDS.Dashboard.handleFundChange("ЗОЛЯ");
// График должен обновиться с новыми данными: 220, 560, 780, 1100

BDDS.Dashboard.handleFundChange("КРАС");
// График: 310, 620, 850, 1200
```
    ...
    "ДВН": {
      ...
      periods: {
        "Факт '23": {
          ...
          "Выплата дохода акционерам (пайщикам)": -150,  // ← данные для графика
        ...
        "Факт '24": {
          // ... другие метрики ...
          "Выплата дохода акционерам (пайщикам)": -480  // ← данные для графика
        },
        "План '26": {
          // ... другие метрики ...
          "Выплата дохода акционерам (пайщикам)": -1350  // ← данные для графика
        }
      }
    },
    "ЗОЛЯ": {
...
BDDS.Dashboard.loadData(data);
// График автоматически отобразит: 150, 480, 1350 млн ₽
```
...
currentData.funds["НовыйФонд"] = {
  ...
  periods: {
    ...
    "План '25": {
      "Поступления по операционной деятельности": 600,
      "Выплата дохода акционерам (пайщикам)": -300
    }
  }
};
